  ...
  for (let attempt = 0; attempt < WALK_IN_CODE_MAX_RETRIES; attempt++) {
    ...
    try {
      booking = await prisma.$transaction(
        ...
        const uniqueRequestedLaneNumbers = new Set(requestedLaneNumbers)
        if (uniqueRequestedLaneNumbers.size !== requestedLaneNumbers.length) {
          throw new Error('Selected lanes must be unique.')
        }

        if (requestedLaneNumbers.length) {
