...
export const CalendarColumnHeader = ({ state, weekDay, day, className, onClick }: CalendarColumnHeaderProps) => {
    return (
        ...
        >
            <span className="text-xs font-medium text-quaternary max-md:hidden">{weekDay}</span>
            <span className="text-xs font-medium text-quaternary md:hidden">{weekDay.charAt(0)}</span>
            {typeof day === "number" && (
                <span
                    className={cx(
